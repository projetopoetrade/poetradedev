...
/**
 * Fetches active leagues from Supabase.
 * Returns slug (generated from name), gameVersion and lastmod for sitemap generation.
 */
async function getSitemapLeaguesFromSupabase() {
  if (!supabaseClient) return [];
  try {
    const { data: leagues, error } = await supabaseClient
      .from('leagues')
      .select('league_slug, gameVersion, updated_at')
      .eq('isActive', true);

    if (error) throw error;
    console.log(`Sitemap: Fetched ${leagues?.length || 0} active leagues from Supabase.`);
    return (leagues || []).filter(l => l.league_slug && l.gameVersion).map(l => ({
      slug: l.league_slug,
      gameVersion: l.gameVersion,
      lastmod: l.updated_at ? new Date(l.updated_at).toISOString() : new Date().toISOString(),
    }));
  } catch (error) {
    console.error('Sitemap: Error fetching leagues from Supabase:', error);
    return [];
  }
}
